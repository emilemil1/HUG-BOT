...
class Watch {
	...
	constructor(builder: PluginBuilder) {
		...
		this.tools.client.on("guildCreate", (guild) => {
			this.cache.set(guild.id, {
				regex: this.buildRegex(this.tools.getConfigs(guild.id)["watch"].data!.matches as {[index: string]:string[]}),
				matches: this.tools.getConfigs(guild.id)["watch"].data!.matches as {[index: string]:string[]},
				silent: this.tools.getConfigs(guild.id)["watch"].data!.silent as {[index: string]:null}
			})
		})
	}
	...
	sync() {
		for (const guild of this.tools.client.guilds.values()) {
			const matches = this.tools.getConfigs(guild.id)["watch"].data!.matches as {[index: string]:string[]};
			const silent = this.tools.getConfigs(guild.id)["watch"].data!.silent as {[index: string]:null};
	...
	constructEmbedMessage(message: Message, replacement: string, index: number) {
		let embedMessage;
	...
	buildRegex(matches: {[index: string]:string[]}) {
		let rx = "";
